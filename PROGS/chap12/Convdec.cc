...
// r is an array of the received values
{
   ...
   for(state = 0; state < numstates; state++) { 
	  ...
	  if(bestcost < d) {		// determine the state with the best metric
		 d = bestcost;
		 beststate = state;
	  }
   ...
   // cout << "beststate=" << beststate << "  d=" << d << endl;
   // swap the metric information
   metswap = metrics;   metrics = othermetrics;  othermetrics = metswap;

   // now see if we are ready to decode (if >= M branches have been decoded 
   if(numbranchdec < pathmem) { // not enough ready to decode
	  numbranchdec++;
	  // move the front of the path list forward
	  fpath = inci(fpath);
	  return(0);
   }
   ...
   bestst_back = paths[fpath].state[beststate];
   i = fpath;
   do {
